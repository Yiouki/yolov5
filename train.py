...
from utils.general import (LOGGER, check_amp, check_dataset, check_file, check_git_status, check_img_size,
                           ...
                           one_cycle, print_args, print_gpu_memory, print_mutation, strip_optimizer, yaml_save)
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg, resume, finetuning, noval, nosave, workers, freeze = \
    ...
    data_dict = None
    if RANK in {-1, 0}:
        loggers = Loggers(save_dir, weights, opt, hyp, LOGGER, recurrent_save=opt.recurrent_save, noplots=opt.noplots)  # loggers instance
    ...
    if RANK == -1 and batch_size == -1:  # single-GPU only, estimate best batch size
        ...
        opt.batch_size_use = batch_size
        yaml_save(save_dir / 'opt.yaml', vars(opt))  # Save with the update of opt values
    ...
    if pretrained:
        if resume:
            best_fitness, start_epoch, epochs = smart_resume(ckpt, optimizer, ema, weights, epochs, resume, finetuning)
        del ckpt, csd
    ...
    if not opt.finetuning and opt.debug_finetuning and not resume:
        ...
        model_save(model, Path(opt.save_dir) / "weights", "debug_weights_init", anchors="anchors_init")
    ...
    if not opt.finetuning and opt.debug_finetuning and not resume:
        model_save(model, Path(opt.save_dir) / "weights", "debug_weights", anchors="anchors")
...
def main(opt, callbacks=Callbacks()):
    ...
    if opt.resume and not check_wandb_resume(opt) and not check_comet_resume(opt) and not opt.evolve:
        ...
        opt.cfg, opt.weights, opt.resume, opt.epochs = '', str(last), True, opt_epochs  # reinstate
    ...
    # Evolve hyperparameters (optional)
        ...
        meta = {
            # 'lr0': (1e-20, 1e-20, 1e-20),  # initial learning rate (SGD=1E-2, Adam=1E-3)
